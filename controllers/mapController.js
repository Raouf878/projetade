...
const getMarkers=asyncHandler(async(req,res)=>{
    const sql = `SELECT acc_ID,accident_start_time,accident_end_time,accident_start_date,accident_end_date,accident_status,population_affected,accident_cost,accident_title,team_name,longtitude,latitude,address,first_name,last_name,accident_name FROM accidents JOIN team ON accidents.team_ID  = team.team_ID JOIN localisation ON accidents.local_ID= localisation.local_ID JOIN users ON accidents.id_user=users.id_user where accidents.id_user=3 `;
    try{
